    ...
    int width, int height) const
{
    ...
    cairo_set_source_rgba(cr, 0.97, 0.97, 0.99, 0.9);
    ...
    const button_state_t& state) const
{
    ...
    cairo_rectangle(cr, 0, 0, state.width+4.0, state.height);
    cairo_fill(cr);
    ...
    /* For US, we need a light color. */
    color_t base = {0.97,0.97,0.99,0.8};
    ...
    cairo_set_source_rgba(cr, 0.97, 0.97, 0.99, line / 2);
    cairo_set_antialias(cr, CAIRO_ANTIALIAS_NONE);
    switch (button)
    {
      case BUTTON_CLOSE:
        cairo_set_line_width(cr, 3 * state.border);
      ...
      case BUTTON_MINIMIZE:
        cairo_set_line_width(cr, 4 * state.border);
        cairo_move_to(cr, 1.0 * state.width / 4.0,
            state.height / 2.0);
        cairo_line_to(cr, 3.0 * state.width / 4.0,
            state.height / 2.0);
